...
const NAV_GROUPS = [
  ...
  {
    label: 'Commerce',
    items: [{ label: 'Produce intake', icon: Inbox }],
  },
  {
    label: 'Communications',
    items: [{ label: 'SMS broadcasts', icon: MessageSquare }],
  },
]
...
export default function DashboardMock() {
  return (
    <div className="mock" aria-label="AgroOS dashboard preview">
      ...
      <div className="mock-body">
        <aside className="mock-side">
          ...
          <nav className="mock-nav" aria-label="Dashboard preview navigation">
          ...
          <div className="mock-content">
            <div className="mock-stats">
              {STATS.map(([label, value, detail]) => (
                <div key={label} className="mock-stat">
                  <div className="mock-stat-l">{label}</div>
                  <div className="mock-stat-v serif">{value}</div>
                  <div className="mock-stat-s">{detail}</div>
                </div>
              ))}
            </div>
            ...
            <div className="mock-grid">
              <div className="mock-card">
                ...
                <div className="mock-th">
                  {['Member', 'Amount', 'Method', 'Date', 'Status'].map((heading) => (
                    <span key={heading} className="mock-tl">{heading}</span>
                  ))}
                </div>
                {PAYMENTS.map(([name, id, amount, method, date, status, badge]) => (
                  <div key={name} className="mock-tr">
                    <div>
                      <div className="mock-tr-n">{name}</div>
                      <div className="mock-tr-id">{id}</div>
                    </div>
                    <span className="mock-tr-v">{amount}</span>
                    <span className="mock-tr-m">{method}</span>
                    <span className="mock-tr-m">{date}</span>
                    <span className={`bdg ${badge}`}>{status}</span>
                  </div>
                ))}
              </div>
            ...
            <div className="mock-card mock-review">
              ...
              <div className="mock-review-grid">
                {REVIEW_QUEUE.map(([name, meta, score, recommendation]) => (
                  <div key={name} className="mock-review-card">
                    ...
                    <div className="mock-review-rec">{recommendation}</div>
                  </div>
                ))}
              </div>
            </div>
