...
export default function BookDemoPage({ setPage }) {
  const [step, setStep] = useState(0) // 0=pick slot, 1=details, 2=confirm
          ...
          {step < 2 ? (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32 }}>
                ...
                <div style={{
                  ...
                  {step === 0 && (
                    <>
                      ...
                      <button className="btn-lg" style={{ width: '100%' }} onClick={goToDetails}>Continue →</button>
                    </>
                  )}
                  ...
                  {step === 1 && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                      ...
                      <div style={{ display: 'flex', gap: 10 }}>
                        <button className="btn-out-lg" style={{ fontSize: 13, padding: '10px 18px', flex: 1 }} onClick={() => { setStep(0); setErr('') }}>← Back</button>
                        <button className="btn-lg" style={{ fontSize: 13, padding: '10px 18px', flex: 2 }} onClick={submitDemo}>Confirm booking →</button>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                {/* Team */}
                <div style={{
                  background: 'var(--sage)', border: '1px solid var(--border)', borderRadius: 14, padding: '18px 20px',
                  ...
                  <div style={{ display: 'flex' }}>
                    {['KA','EM','NB'].map((init, i) => (
                      <div key={init} style={{
                        ...
                        background: ['#1a4731','#2d6a4f','#52b788'][i],
                        color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center',
                        fontSize: 12, fontWeight: 700, border: '2px solid var(--sage)',
                        marginLeft: i > 0 ? -10 : 0, zIndex: 3-i, position: 'relative',
                      }}>{init}</div>
                    ))}
                  </div>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 600 }}>Meet the AgroOS team</div>
                    <div style={{ fontSize: 11, color: 'var(--muted)' }}>We're based in Accra, Ghana 🇬🇭</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div style={{ maxWidth: 500, margin: '0 auto', textAlign: 'center' }}>
              ...
              <div style={{
                background: 'var(--sage)', borderRadius: 12, padding: '16px 20px', marginBottom: 28,
              ...
              <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
                <button className="btn-out-lg" style={{ fontSize: 13, padding: '10px 22px' }} onClick={() => setPage('home')}>Back to home</button>
                <button className="btn-lg" style={{ fontSize: 13, padding: '10px 22px' }} onClick={() => setPage('dashboard')}>Try dashboard →</button>
              </div>
            </div>
          )}
        </div>
